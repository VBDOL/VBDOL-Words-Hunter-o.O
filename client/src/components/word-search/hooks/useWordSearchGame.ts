  ...
  const checkForWord = () => {
    ...
    if (foundWordPosition && !foundWords.some(fw => fw.word === foundWordPosition.word)) {
      ...
      if (isValidBonusWord) {
        ...
        console.log(`Bonus word found: ${bonusWord} (+${bonusTime}s)`);
        ...
        // Only count as mistake if it's not a valid bonus word AND has enough letters
        if (selectedCellsArray.length >= 3) {
          setMistakes(prev => prev + 1);
          console.log(`Invalid word attempt: ${word} (${selectedCellsArray.length} letters)`);
        }
      }
    }
